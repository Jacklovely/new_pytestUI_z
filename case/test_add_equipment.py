...
class Test_Add_Equipment():
    ...
    @allure.story("用例：添加设备")
    @pytest.mark.parametrize("name,model,num,brand,version,msg", testdata["equipment_fail_data"],
                             ids=["添加设备，断言失败"])
    # @pytest.mark.skip('跳过该成功用例')
    def test_add_equipment_2(self, login_fixtrue, name, model, num, brand, version,msg):
        driver = login_fixtrue
        register = Add_Equipment_Page(driver)
        with allure.step("点击添加设备,跳转添加设备"):
            time.sleep(1)
